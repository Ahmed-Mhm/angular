...
import { Productdepartment, DataTablesResponse, ProductdepartmentDto } from "./productdepartment.model";
...
@Component({
  selector: "app-allproductsdepartment",
  templateUrl: "./allproductsdepartment.component.html",
  styleUrls: ["./allproductsdepartment.component.sass"],
})
export class AllproductdepartmentComponent
  extends UnsubscribeOnDestroyAdapter
  implements OnInit {
  exampleDatabase: ProductdepartmentService | null;
  selection = new SelectionModel<ProductdepartmentDto>(true, []);
  index: number;
  id: string;
  productdepartment: ProductdepartmentDto | null;
  ...
  ngOnInit() {
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        this.httpClient
          .post<any>(
            this.API_URL + 'v1/productdepartment/getdatatable',
  ...
  addNew() {
    ...
    const dialogRef = this.dialog.open(FormDialogComponent, {
      data: {
        productdepartment: this.productdepartment,
        action: "add",
      },
  ...
  editCall(id) {
    ...
    this.productdepartmentService.getEditProductdepartment(id).subscribe((res) => {
      const dialogRef = this.dialog.open(FormDialogComponent, {
        data: {
          productdepartment: res.data,
          productdepartmentId: id,
          action: "edit",
        },
      direction: tempDirection,
      disableClose: true,
